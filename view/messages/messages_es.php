<?php
// file: /view/messages/messages_es.php
$i18n_messages = array(
    "Users Login" => "Iniciar sesion",
    "Please enter your login and password" => "Por favor introduce tu usuario y contraseña",
    "Login here" => "Usuario aqui",
    "Password here" => "Contraseña aqui",
    "Register here!" => "Registrate aqui!",
    "Iniciar sesion" => "Iniciar sesión",
    "nombre" => "nombre",
    "apellidos" => "apellidos",
    "rol" => "rol",
    "genero" => "genero",
    "Usuarios" => "Usuarios",
    "Opciones" => "Opciones",
    "femenino" => "femenino",
    "masculino" => "masculino",
    "Editar" => "Editar",
    "Eliminar" => "Eliminar",
    "¿Estas seguro de eliminar a este usuario?" => "¿Estas seguro de eliminar a este usuario?",
    "a" => "administrador",
    "e" => "entrenador",
    "d" => "deportista",
    "Crear" => "Crear",
    "Campeonato" => "Campeonato",
    "Inscripción" => "Inscripción",
    "Gestionar Resultados" => "Gestionar resultados",
    "Ver clasificacion" => "Ver clasificacion",
    "Partido" => "Partido",
    
    "Match successfully organize" => "Partido organizador correctamente",
    "Organize Match" => "Organizar partido",
    "View Organized Matches" => "Ver partidos organizados",
    "Date" => "Fecha",
    "IDMatch" => "ID Partido",
    "Time" => "Hora",
    "View Organized Matches" => "Ver partidos organizados",
    "Participants" => "Participantes",
    "Join" => "Unirse",
    "User Login is mandatory" => "Nombre de usuario es obligatorio",
    "Id Participantes Partido is mandatory" => "ID participantes partido es obligatorio",
    "Id Organizar Partido is mandatory" => "ID organizar partido es obligatorio",
    "Generate championship calendar" => "Generar calendario de campeonato",
    "Reserve court" => "Reservar pista",
    "Select schedule" => "Seleccionar horario",
    "Reserve" => "Reservar",
    "Track reserved" => "Pista reservada",
    "Select championship" => "Seleccionar campeonato",
    "Generate calendar" => "Generar calendario",
    "View confrontations" => "Ver enfrentamientos",
    "Confrontations" => "Enfrentamientos",
    "Properly generated calendar." => "Calendario generado correctamente.",
    "There are not enough athletes register to create a group." => "No hay suficientes deportistas apuntados para crear un grupo.",
    "Create championship" => "Crear campeonato",
    "Championship name" => "Nombre de campeonato",
    "Registration start date" => "Fecha inicial de registro",
    "Registration limit date" => "Fecha límite de registro",
    "Championship start date" => "Fecha de inicio del campeonato",
    "Championship finish date" => "Fecha fin del campeonato",
    "Register championship" => "Registrar campeonato",
    "Select categories" => "Seleccionar categorías",
    "Add categories" => "Añadir categorias",
    "Sign up for championship" => "Inscribirse a campeonato",
    "Sign up" => "Incribirse",
    "Championship" => "Campeonato",
    "Category" => "Categoría",
    "Couple username" => "Nombre de pareja",
    "Enter match results" => "Introducir resultados de los partidos",
    "Save" => "Guardar",
    "Manage results" => "Gestionar resultados",
    "Show clasification" => "Ver clasificacion"
    "Clasification" => "Clasificación",
    "Position" => "Posición",
    "Couple" => "Pareja",
    "Total points" => "Puntos totales",
    "Total sets" => "Sets totales",

    "There are not enough athletes register to create a group." => "No hay suficientes deportistas apuntados para crear un grupo",

    "Inscripción" => "Inscripción",
    "Login" => "Usuario",
    "No hay sesión, para inscribir una pareja requiere estar logeado" => "No hay sesión, para inscribir una pareja requiere estar logeado",
    "Submit" => "Enviar",
    "El login de tu pareja no puede ser igual a tu login" => "El login de tu pareja no puede ser igual a tu login",
    "El login no existe" => "El login no existe",
    "Inscripción con exito" => "Inscripción con exito",
    "El genero de tu pareja no es compatible con esta categoria" => "El genero de tu pareja no es compatible con esta categoria",
    "Tu genero no es compatible con esta categoria" => "Tu genero no es compatible con esta categoria",
    "Tu compañero ya esta inscrito en esta categoría" => "Tu compañero ya esta inscrito en esta categoría",
    "Ya estas inscrito en esta categoría" => "Ya estas inscrito en esta categoría",
    "error en peticion." => "error en peticion.",
    "idCapitan es obligatorio" => "idCapitan es obligatorio",
    "idCompañero es obligatorio" => "idCompañero es obligatorio",
    "idCategoriaCampeonato es obligatorio" => "idCategoriaCampeonato es obligatorio",
    "compañero no es valido" => "compañero no es valido",
    "Selecionar Campeonato" => "Selecionar Campeonato",
    "Categoria" => "Categoria",
    "Grupo" => "Grupo",
    "Clasificación" => "Clasificación",
    "puesto" => "Puesto",
    "Pareja" => "Pareja",
    "Puntos Totales" => "Puntos Totales",
    "Introducir resultados" => "Introducir resultados",
    "Fecha" => "Fecha",
    "Hora" => "Hora",
    "Pareja1" => "Partner1",
    "Pareja2" => "Partner2",
    "sets Pareja1" => "sets Pareja1",
    "sets Pareja2" => "sets Pareja2",
    "Enfrentamientos" => "Enfrentamientos",
    "Inscripción" => "Inscripción"
)?>
