...
* add (User)
*
* Vista que muestra un formulario para añadir usuarios 
* 
*
*/
...
<form action="index.php?controller=users&amp;action=add" method="POST">
		...
		<div class="row justify-content-center">
			<div class="col-sm-6 col-xs-12">
				<div class="form-group">
					<label for="login"> <?= i18n("Login") ?> </label> <input
						type="text" class="form-control" id="login" name="login"
						aria-describedby="loginHelp"
						placeholder="<?= i18n("Enter login") ?>"
						value="<?= $user->getLogin() ?>">
    <?= isset($errors["login"])?i18n($errors["login"]):"" ?><br>
				</div>

				<div class="form-group">
					<label for="username"> <?= i18n("Name") ?> </label> <input
    ...
    <?= isset($errors["pass"])?i18n($errors["pass"]):"" ?><br>
				...
				<div class="form-group">
					<label for="gender" size="1"> <?= i18n("Gender") ?> </label> <select
				...
				<button type="submit" class="btn btn-primary"
					value="<?= i18n("Add user")?>"><?= i18n("Add user")?></button>
			</div>
		</div>
	</div>
</form>
